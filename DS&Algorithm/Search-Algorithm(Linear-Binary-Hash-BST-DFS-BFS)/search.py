...
##############################################################################################################################################################                                                                            
#################################### 배열 구현 중도 포기 (삭제 알고리즘에서 두 개의 자식 노드가 다 존재할 경우가 구현이 어려움) #####################################
##############################################################################################################################################################
...
# 4-4. 이진 탐색 트리 (삭제2) -> 배열을 다 앞으로 당기는 알고리즘
...
# dfs (반복문 stack 사용, dictionary) - 맨 오른쪽부터 dfs
def i_dfs(v):
    visited[v] = True
    stack = []
    stack.append(v)
...
if __name__ == '__main__':
    ...
    arr = [12, 25, 36, 20, 30, 8, 42]
    key = int(input())

    linear_search(arr, key)
    binary_search(arr, key)
    hash_search_save(arr, key)

    ####################################
    ## 배열로 만든 bst -> 삭제기능 미완 ##
    #####################################
    binary_search_tree_arr(key)

    #####################################    
    ## Liked List로 만든 bst ##
    #####################################
    binary_search_tree_link(key)
    ...
    # recursive dictionary dfs
    r_dfs(1)
    # iterative dictionary dfs
    i_dfs(1)